...
function FlagService:ApplyFlag(player: Player, flagName: string, duration: number?, source: any?, data: any?): boolean
	if flagName == "Burn" or flagName == "Poison" or flagName == "Stun" or flagName == "Petrify" then print(`[PET_ATTACK_TRACE][FlagService] ApplyFlag entered target={player.Name} flag={flagName} duration={tostring(duration)} source={getSourceId(source, data)}`) end
	...
	if not state then
		if flagName == "Burn" or flagName == "Poison" or flagName == "Stun" or flagName == "Petrify" then print(`[PET_ATTACK_TRACE][FlagService] ApplyFlag aborted target={player.Name} flag={flagName}: state missing`) end
		return false
	end
	...
	elseif flagName == "Stun" and self:HasFlag(player, "Petrify") then
		print(`[PET_ATTACK_TRACE][FlagService] ApplyFlag aborted target={player.Name} flag=Stun: Petrify already active`)
		return false
	end
	local defaults = getFlagDefaults(flagName)
	local resolvedDuration = math.max(0, duration or defaults.Duration or 0)
	if resolvedDuration <= 0 then
		if flagName == "Burn" or flagName == "Poison" or flagName == "Stun" or flagName == "Petrify" then print(`[PET_ATTACK_TRACE][FlagService] ApplyFlag aborted target={player.Name} flag={flagName}: resolvedDuration={resolvedDuration}`) end
		return false
	end
	local flags = self._activeFlags[player]
	if not flags then
	...
	if flagName == "Burn" or flagName == "Poison" or flagName == "Stun" or flagName == "Petrify" then print(`[PET_ATTACK_TRACE][FlagService] ApplyFlag success target={player.Name} flag={flagName} stacks={stacks} expiresAt={expiresAt}`) end
	return true
end
