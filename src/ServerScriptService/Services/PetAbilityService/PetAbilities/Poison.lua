...
function Dot.OnCollision(context, collisionType: string, target: any, _payload: any)
	print(string.format("[PET_ATTACK_TRACE][Poison] OnCollision fired attacker=%s type=%s target=%s", context.player.Name, collisionType, target and target.Name or "nil"))
	if collisionType == "Player" and target then
		if not EffectUtil.CanAffectPlayers(context, context.player, target) then
			print("[PET_ATTACK_TRACE][Poison] OnCollision aborted: target cannot be affected")
			return
		end
