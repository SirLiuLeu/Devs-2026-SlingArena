...
function PlayerService:ShowHpBarHealAmount(player: Player, amount: number)
	if amount <= 0 then
		return
	end
	...
	local existing = hpBarBackground:FindFirstChild("LevelUpHealAmount")
	if existing then
		existing:Destroy()
	end

	local label = Instance.new("TextLabel")
	label.Name = "LevelUpHealAmount"
