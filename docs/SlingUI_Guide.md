...
3. The `SlingUI` ScreenGui becomes visible (`JoystickRoot`, `ChargeBar`, `DirectionArrow`).
...
- Required hierarchy: `SlingUI (ScreenGui) -> JoystickRoot -> Base, Thumb`, plus `ChargeBar -> Fill` and `DirectionArrow`.
...
- **Joystick not appearing** → input start is not hitting the player Sling, or the `SlingUI` hierarchy path is wrong.
...
- **Food floating** → the food model pivot is centered instead of bottom-aligned, so spawn code must apply one bottom-offset correction instead of adding arbitrary Y offsets per spawn.

## 5. Debug logging currently enabled

Temporary debug prints were added for:

- input start
- delta movement
- charge percent
- max charge reached
- food spawn anchor position
- food root position after spawn

Remove or gate these logs later if the behavior is confirmed stable in Studio playtests.
